# Docker for uv-project-ty-ruff-bitbucket-streamlit
...
Default name is `IMAGE := uv_project_ty_ruff_bitbucket_streamlit`.
...
docker run -it --rm \
   -v $(pwd):/workspace \
   uv_project_ty_ruff_bitbucket_streamlit bash
```
